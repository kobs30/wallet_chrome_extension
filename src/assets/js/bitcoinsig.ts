...
import { Bitcoin, BigInteger, getSECCurveByName, ECPointFp, Crypto } from './bitcoinjs-lib';
...
export function verify_message(signature, message, addrtype) {
  try {
...
export function sign_message(private_key, message, compressed, addrtype) {
  ...
  for (var i = 0; i < 4; i++) {
    ...
    if (verify_message(sig, message, addrtype) == address) return sig;
  }
