...
export function getPasswordStrength(value: string): PasswordStrengthMeterStrength;
export function getPasswordStrength(value: string): number {
  let res = 0;
  if (value.length >= 1) res += 1;
  if (value.length >= 6) res += 1;
  if (value.length >= 8) res += 1;
  if (/\d/.test(value)) res += 1;
  if (SYMBOLS_REGEXP.test(value)) res += 1;
  if (value !== value.toLowerCase()) res += 1;
  return res;
}
